...
const DADOS_INICIAIS: ProjectionRow[] = [
  { period: 'M1', maquinas: '1 A1', faturamento_bruto: 1650.00, cmv_insumos: 347.48, taxas_comissoes: 33.00, opex_fixo: 0.00, lucro_operacional: 1269.52, capex: 0.00, caixa_acumulado: 1269.52 },
  { period: 'M2', maquinas: '1 A1', faturamento_bruto: 3300.00, cmv_insumos: 694.97, taxas_comissoes: 264.00, opex_fixo: 0.00, lucro_operacional: 2341.04, capex: 0.00, caixa_acumulado: 3610.55 },
  { period: 'M3', maquinas: '1 A1', faturamento_bruto: 4950.00, cmv_insumos: 1042.45, taxas_comissoes: 574.20, opex_fixo: 0.00, lucro_operacional: 3333.35, capex: 0.00, caixa_acumulado: 3941.91 },
  { period: 'M4', maquinas: '1 A1', faturamento_bruto: 5005.00, cmv_insumos: 1054.03, taxas_comissoes: 640.64, opex_fixo: 954.00, lucro_operacional: 2356.33, capex: 1784.94, caixa_acumulado: 4513.29 },
  { period: 'M5', maquinas: '1 A1', faturamento_bruto: 5005.00, cmv_insumos: 1054.03, taxas_comissoes: 700.70, opex_fixo: 1024.00, lucro_operacional: 2226.27, capex: 1784.94, caixa_acumulado: 4814.81 },
  { period: 'M6', maquinas: '2 A1', faturamento_bruto: 9900.00, cmv_insumos: 2084.90, taxas_comissoes: 1504.80, opex_fixo: 1024.00, lucro_operacional: 5286.31, capex: 4675.00, caixa_acumulado: 3641.17 },
  { period: 'M7', maquinas: '2 A1', faturamento_bruto: 10010.00, cmv_insumos: 2108.06, taxas_comissoes: 1641.64, opex_fixo: 1024.00, lucro_operacional: 5236.30, capex: 1784.94, caixa_acumulado: 7092.53 },
  { period: 'M8', maquinas: '3 A1', faturamento_bruto: 14300.00, cmv_insumos: 3011.52, taxas_comissoes: 2345.20, opex_fixo: 1024.00, lucro_operacional: 7919.29, capex: 4675.00, caixa_acumulado: 5883.76 },
  { period: 'M9', maquinas: '4 A1', faturamento_bruto: 16500.00, cmv_insumos: 3474.83, taxas_comissoes: 2706.00, opex_fixo: 1024.00, lucro_operacional: 9295.18, capex: 1784.94, caixa_acumulado: 4422.14 },
  { period: 'M10', maquinas: '4 A1', faturamento_bruto: 18700.00, cmv_insumos: 3938.14, taxas_comissoes: 3066.80, opex_fixo: 1054.00, lucro_operacional: 10641.07, capex: 1784.94, caixa_acumulado: 8976.63 },
  { period: 'M11', maquinas: '5 A1', faturamento_bruto: 21450.00, cmv_insumos: 4517.27, taxas_comissoes: 3517.80, opex_fixo: 1054.00, lucro_operacional: 12360.93, capex: 3552.04, caixa_acumulado: 7438.81 },
  { period: 'M12', maquinas: '6 A1', faturamento_bruto: 24750.00, cmv_insumos: 5212.24, taxas_comissoes: 3910.50, opex_fixo: 1054.00, lucro_operacional: 14573.26, capex: 0.00, caixa_acumulado: 6892.52 }
]
  ...
  const handleSalvarNoSupabase = async () => {
    try {
      ...
      for (const item of registros) {
        // Removemos o ID caso seja numérico simples para evitar o conflito de tipo UUID do banco
    ...
    <main className="max-w-[98vw] mx-auto space-y-6 p-4 md:p-6 pb-20 text-slate-100">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-[#0b1426] p-6 rounded-3xl border border-white/10 shadow-2xl">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {loading ? 'Sincronizando...' : 'Salvar Alterações no Supabase'}
          </button>
          <Link
            href="/financeiro"
        ...
        <div className="bg-[#0b1426] rounded-3xl border border-white/10 overflow-hidden shadow-2xl">
          <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/40">
            ...
            <button 
              ...
              className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold flex items-center gap-1.5 transition border border-slate-700 cursor-pointer"
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-slate-800/60 font-medium text-slate-200">
                ...
                <tr className="bg-slate-900/20">
                  <td className="p-4 font-bold text-amber-300">(-) OPEX Fixo Reajustado (R$)</td>
                  {registros.map((item) => (
                    <td key={item.period} className="p-2 text-center">
                      <input
                        type="number"
                ...
                <tr>
                  <td className="p-4 font-bold text-purple-400">(-) CAPEX / Aquisições (R$)</td>
                  {registros.map((item) => (
                    <td key={item.period} className="p-2 text-center">
                      <input
                        type="number"
      ...
      {abaAtiva === 'capex' && (
        <div className="bg-[#0b1426] p-8 rounded-3xl border border-white/10 shadow-2xl space-y-4">
          ...
          <p className="text-xs text-slate-400">Gerenciamento dos aportes em maquinário da Print Farm.</p>
      ...
      {abaAtiva === 'opex' && (
        <div className="bg-[#0b1426] p-8 rounded-3xl border border-white/10 shadow-2xl space-y-4">
          ...
          <p className="text-xs text-slate-400">Controle das despesas fixas recorrentes da operação.</p>
          <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700 text-xs text-slate-300 space-y-1">
      ...
      {abaAtiva === 'cmv' && (
        <div className="bg-[#0b1426] p-8 rounded-3xl border border-white/10 shadow-2xl space-y-4">
          <h3 className="text-lg font-black text-white">Aba CMV & Insumos</h3>
          <p className="text-xs text-slate-400">Custo de Mercadoria Vendida voltado ao consumo de filamentos e energia.</p>
          <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700 text-xs text-slate-300 space-y-1">
      ...
      {abaAtiva === 'taxas' && (
        <div className="bg-[#0b1426] p-8 rounded-3xl border border-white/10 shadow-2xl space-y-4">
          ...
          <p className="text-xs text-slate-400">Descontos de gateways e marketplaces.</p>
          <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700 text-xs text-slate-300 space-y-1">
